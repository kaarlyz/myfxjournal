    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          onClick={fetchData}
          disabled={loading}
          className="p-2 bg-gray-800 hover:bg-gray-700 text-gray-300 rounded-lg text-xs font-semibold flex items-center space-x-1.5 transition disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="glass-card rounded-2xl p-5 border border-gray-800 space-y-5">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-gray-500 uppercase block">URL Webhook</label>
              <div className="flex bg-gray-950 border border-gray-850 rounded-lg overflow-hidden p-1">
                <input
                  type="text"
                  readOnly
                  value={webhookUrl}
                  className="bg-transparent flex-1 text-xs outline-none px-2 font-mono text-gray-300"
                />
                <button
                  onClick={() => copyToClipboard(webhookUrl, 'url')}
                  className="p-1.5 bg-gray-900 hover:bg-gray-800 rounded text-gray-400 hover:text-white transition"
                >
                  {copiedText === 'url' ? <span className="text-[10px] text-accentEmerald font-semibold px-1">Copied!</span> : <Copy className="w-3.5 h-3.5" />}
                </button>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-gray-500 uppercase block">Secret Token Webhook</label>
              <div className="flex bg-gray-950 border border-gray-850 rounded-lg overflow-hidden p-1">
                <input
                  type="text"
                  readOnly
                ...
                <button
                  ...
                  className="p-1.5 bg-gray-900 hover:bg-gray-800 rounded text-gray-400 hover:text-white transition"
                >
                  {copiedText === 'token' ? <span className="text-[10px] text-accentEmerald font-semibold px-1">Copied!</span> : <Copy className="w-3.5 h-3.5" />}
                </button>
              </div>
              <p className="text-[10px] text-gray-500 flex items-start space-x-1">
            ...
            {summary && (
              <div className="border-t border-gray-850 pt-4 grid grid-cols-3 gap-2 text-center text-xs">
                <div className="bg-accentEmerald/5 border border-accentEmerald/10 rounded-lg py-2">
                  <span className="text-[10px] text-gray-500 block uppercase">Sukses</span>
                  <span className="font-bold text-accentEmerald">{summary.summaryCounts.success}</span>
                </div>
                <div className="bg-orange-500/5 border border-orange-500/10 rounded-lg py-2">
                  <span className="text-[10px] text-gray-500 block uppercase">Orphan</span>
                ...
                <div className="bg-lossRed/5 border border-lossRed/10 rounded-lg py-2">
                  <span className="text-[10px] text-gray-500 block uppercase">Error</span>
                  <span className="font-bold text-lossRed">{summary.summaryCounts.error}</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="glass-card rounded-2xl p-5 border border-gray-800 space-y-4">
            ...
            <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
              {!summary || summary.openTrades.length === 0 ? (
                <p className="text-xs text-gray-600 italic py-4 text-center">Tidak ada trade open.</p>
              ) : (
                summary.openTrades.map((t) => (
                  <div key={t.id} className="bg-accentCyan/5 border border-accentCyan/15 rounded-xl p-3 space-y-2">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-[9px] font-bold bg-accentCyan/20 text-accentCyan px-1.5 py-0.5 rounded animate-pulse">
                        {t.side}
                      </span>
                    </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="glass-card rounded-2xl p-5 border border-gray-800 space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center space-x-1.5">
                <Terminal className="w-4 h-4 text-accentEmerald" />
                <span>Contoh Pine Script v5</span>
              </h3>
              <button
                onClick={() => copyToClipboard(pineScriptCode, 'pine')}
                className="px-2.5 py-1 bg-gray-800 hover:bg-gray-700 text-gray-300 rounded text-xs font-semibold flex items-center space-x-1.5 transition"
              >
                {copiedText === 'pine' ? <span>Copied!</span> : <span>Copy Kode</span>}
              </button>
            </div>
            <pre className="bg-gray-950 p-4 rounded-xl text-[10px] font-mono text-gray-400 overflow-x-auto border border-gray-850 max-h-48 leading-normal">
              {pineScriptCode}
            </pre>
          </div>
          ...
          <div className="glass-card rounded-2xl border border-gray-800 overflow-hidden">
            <div className="p-5 border-b border-gray-800/80 bg-gray-900/10 flex justify-between items-center">
              ...
              <span className="text-[9px] bg-gray-800 px-2 py-0.5 rounded text-gray-500 font-semibold uppercase">
            ...
            <div className="p-4 max-h-[300px] overflow-y-auto">
              {events.length === 0 ? (
                <p className="text-center text-xs text-gray-600 py-12 italic">Belum ada request webhook yang masuk.</p>
              ) : (
                        ...
                        <div className="flex justify-between items-center">
                          <div className="flex items-center space-x-2">
                            <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${
                              isSuccess 
                                ? 'bg-accentEmerald/25 text-accentEmerald' 
                                : isOrphan 
                                ? 'bg-orange-500/25 text-orange-400' 
                                : 'bg-lossRed/25 text-lossRed'
                            }`}>
                              {e.status}
                            </span>
                          ...
                          <span className="text-[10px] text-gray-500">{formatDate(e.receivedAt)}</span>
                        </div>

                        {!isSuccess && e.errorMessage && (
                          <p className="text-[11px] text-lossRed font-semibold flex items-center space-x-1">
                            <AlertTriangle className="w-3.5 h-3.5 shrink-0" />
                            <span>{e.errorMessage}</span>
                          </p>
                        )}

                        <details className="text-[10px] text-gray-500 font-mono">
                          <summary className="cursor-pointer hover:text-gray-300 transition outline-none select-none font-semibold">
                            Tampilkan Raw Payload JSON
                          </summary>
                          <div className="mt-1.5 p-2 bg-black/40 rounded border border-gray-900 overflow-x-auto whitespace-pre-wrap leading-normal">
