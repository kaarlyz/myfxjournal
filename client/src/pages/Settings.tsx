    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {message && (
        <div className={`rounded-xl p-4 flex items-start space-x-3 text-xs font-semibold ${
          message.type === 'success' 
            ? 'bg-accentEmerald/10 border border-accentEmerald/20 text-accentEmerald' 
            : 'bg-lossRed/10 border border-lossRed/20 text-lossRed'
        }`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <form onSubmit={handleSave} className="glass-card rounded-2xl border border-gray-800 p-6 space-y-5">
            <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center space-x-1.5">
              <SettingsIcon className="w-4 h-4 text-accentCyan" />
              <span>General Defaults</span>
            </h3>

            {/* USD IDR Exchange Rate */}
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-gray-500 uppercase block">Kurs USD ke IDR Manual</label>
              <input
                type="number"
                value={usdIdrRate}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-gray-500 uppercase block">Model Resiko Bawaan (R)</label>
                <select
        ...
        <div className="space-y-6">
          {/* Seed Data Tool */}
          <div className="glass-card rounded-2xl border border-gray-800 p-5 space-y-4">
            <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center space-x-1.5">
              <Database className="w-4 h-4 text-accentEmerald" />
            ...
            <button
              ...
              className="w-full py-2.5 bg-gradient-to-r from-accentEmerald/20 to-accentCyan/20 hover:from-accentEmerald/30 hover:to-accentCyan/30 border border-accentEmerald/30 text-accentEmerald hover:text-white rounded-xl text-xs font-bold flex items-center justify-center space-x-1.5 transition"
            >
          ...
          {/* Hard Reset database */}
          <div className="glass-card rounded-2xl border border-rose-500/20 p-5 space-y-4 bg-gradient-to-br from-rose-500/[0.02] to-transparent">
            <h3 className="text-xs font-extrabold text-lossRed uppercase tracking-widest flex items-center space-x-1.5">
              <AlertTriangle className="w-4 h-4 text-lossRed" />
              <span>Zona Bahaya (Hard Reset)</span>
            </h3>
            <p className="text-xs text-gray-400 leading-relaxed">
              Tindakan ini akan <span className="font-semibold text-lossRed">menghapus semua sesi backtest, riwayat trade, webhook, dan logs</span> dari database SQLite secara permanen.
            </p>

            <div className="space-y-2 border-t border-gray-850 pt-3">
              <label className="text-[9px] font-bold text-gray-500 uppercase block">
                Ketik tulisan <span className="text-lossRed font-extrabold">"HAPUS"</span> untuk mengonfirmasi:
              </label>
              <input
                type="text"
                ...
                className="w-full bg-gray-900 border border-gray-800 focus:border-rose-500/50 outline-none rounded-lg p-2 text-xs text-gray-200 font-semibold"
              />
            </div>
            ...
            <button
              ...
              className="w-full py-2.5 bg-lossRed hover:bg-lossRed/95 text-white rounded-xl text-xs font-bold flex items-center justify-center space-x-1.5 transition rose-glow disabled:opacity-30 disabled:cursor-not-allowed disabled:shadow-none"
            >
